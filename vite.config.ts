...
const INVOICES_PORT = process.env.NODE_ENV === `production` ? LOCAL_ENV_PORT : 4008
...
export default defineConfig({
  ...
  // `/` for local docker
  // `/invoices` for local-env and prod
  base: `/invoices`,
  plugins: [
  ...
  define: {
    ...
    // In production, it will be `/invoices`, while in development it will be an empty string.
    // Example usage in HTML: <script src="%VITE_BASE_PATH%/env-config.js"></script>
    'import.meta.env.VITE_BASE_PATH': JSON.stringify(
      process.env.NODE_ENV === `production` ? `/invoices` : ``,
    ),
  },
  // Build configuration
  build: {
